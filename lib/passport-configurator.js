...
PassportConfigurator.prototype.configureProvider = function(name, options) {
                ...
                function login(creds) {
                  self.userModel.login(creds,
                    function(err, accessToken) {
                      ...
                      if (accessToken && (!options.emailVerificationRequired || user.emailVerified)) {
            ...
            if (req.user) {
              self.userCredentialModel.link(
                req.user.id, provider, authScheme, profile,
                {
                  accessToken: accessToken,
                  refreshToken: refreshToken,
                }, options, function(err, userCred) {
                  cb(err, req.user, userCred, req.accessToken.id);
                });
            } else {
              // NOTE: req.user ends up being undefined here, even if the client
              // passed a valid access token in the authorization header.
              // For that reason we look up the user by access token here
              self.userModel.findById(req.accessToken.userId, function(err, user) {
                if (err || !user) {
                  console.error("User not found: ", err);
                  done("user not found");
                }
                // NOTE: If a user has already created a 3rd-party userIdentity (and an account)
                // for this 3rd party profile, which of these should we do?
                // a) Link the credential to the logged in user
                // b) Remove the user identity record?
                // c) Unlink the user-identity record (i.e. by assigning it a non existent userid)
                // d) Remove the user account that was created based on this 3rd party profile? (relates to b)
                self.userCredentialModel.link(
                  user.id, provider, authScheme, profile,
                  {
                    accessToken: accessToken,
                    refreshToken: refreshToken
                  }, options, function(err, userCred) {
                    cb(err, user, userCred, req.accessToken.id);
                  });
              });
            }
          } else {
            // See if this user has already been linked before creating an identity
            var cb = loginCallback(req, done);
            var OptionsForCreation = _.defaults({
              autoLogin: true,
            }, options);
            console.log("looking up linked creds for provider", provider, "extid", profile.id);
            self.userCredentialModel.login(provider, authScheme, profile, {},
              OptionsForCreation, function(err, user, identity, token) {
                // if we couldn't find a linked login, then create a user account (and identity)
                // and log it in
                if (err || !user || !identity || !token) {
                  console.log("Unable to find linked account for credentials, creating new one");
                  self.userIdentityModel.login(provider, authScheme, profile,
                    { accessToken: accessToken, refreshToken: refreshToken },
                    options, cb);
                }
  ...
  // Ref: https://github.com/julien-sarazin/loopback-component-passport/blob/57da541be0e25ad18c7b75c1538ce2d5ed977221/lib/passport-configurator.js
        ...
        if (info && info.accessToken) {
          if (!!options.json) {
            return res.json({
              ...
              userId: user.id,
              subscriptionTier: user.subscriptionTier ? user.subscriptionTier : 0
            });
          }
  ...
  if (authType === 'local' || authType === 'ldap') {
    ...
    self.app.post(authPath, callback);
  } else if (link) {
 ...
 /**
   ...
   */
   function addCookies(req, res, user_id, access_token, ttl) {
     var smartTTL = ttl > 0 ? 1000 * ttl : 1000 * (60 * 60 * 24 * 365);
     // ^ 1 yr if eternal (-1) since front-end doesn't handle maxAge=-1
     console.log("Setting cookies!!!, ttl => ", smartTTL);
